  ...
  setInterval(async function () {
    ...
    try {
        ...
        if (stuff.length > 2) {
          ...
          let abear = Math.round(stuffbear[stuffbear.length - 1]* 1000000) / 10000
          let abull = Math.round(stuff[stuff.length - 1]* 1000000) / 10000
        let upd =  await update({epoch: currentEpoch.toNumber(),bull: abull,bear:abear}, 
          { predictions: predictions, auth: wallet.publicKey, systemProgram: new PublicKey("11111111111111111111111111111111") }
          );
          console.log(upd)
tx.add(upd)
      try { 
